package com.restassured.keywords;
...
public class ApiKeyword {
    ...
    @Step("Get Response value by key: {1}")
    public static String getResponseIdValue(String responseBody, String responseKey) {
        JsonPath jsonPath = new JsonPath(responseBody);
        String id_value = jsonPath.get(responseKey).toString();
        LogUtils.info("Value by key (" + responseKey + "): " + id_value);
        AllureManager.saveTextLog("Value by key (" + responseKey + "): " + id_value);
        return id_value;
    }
    ...
    @Step("Verify ID not null: {1}")
    public static void verifyIdNotNull(Response response, String responseIdValue) {
        String actualIdValue = getResponseKeyValue(response, responseIdValue);
        LogUtils.info("Verify Key Value: " + actualIdValue);
        AllureManager.saveTextLog("Verify Status code: " + actualIdValue);
        Assert.assertNotNull(actualIdValue, "FAIL. The ID value is null.");
    }
}
